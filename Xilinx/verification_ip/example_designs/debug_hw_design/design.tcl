...
proc create_ipi_design { offsetfile design_name } {
	...
	# Create instance: verification_ip_0, and set properties
	set verification_ip_0 [ create_bd_cell -type ip -vlnv user.org:user:verification_ip:1.0 verification_ip_0 ]
	...
	# Connect all clock, reset & status pins of verification_ip_0 master interfaces..
	connect_bd_intf_net [get_bd_intf_pins axi_mem_interconnect/S00_AXI] [get_bd_intf_pins verification_ip_0/M00_AXI]
	connect_bd_net [get_bd_pins verification_ip_0/m00_axi_aclk] [get_bd_pins sys_clk_0/clk_out1]
	connect_bd_net [get_bd_pins verification_ip_0/m00_axi_aresetn] [get_bd_pins sys_reset_0/peripheral_aresetn]
	connect_bd_net [get_bd_pins verification_ip_0/m00_axi_txn_done] [get_bd_pins xlconcat_0/In0]
	connect_bd_net [get_bd_pins verification_ip_0/m00_axi_error] [get_bd_pins xlconcat_0/In1]
	connect_bd_net [get_bd_pins verification_ip_0/m00_axi_init_axi_txn] [ get_bd_pins axi_gpio_out/gpio_io_o ]

	# Connect all clock & reset of verification_ip_0 streaming interfaces..
	connect_bd_net [get_bd_pins verification_ip_0/s00_axis_aclk] [get_bd_pins sys_clk_0/clk_out1]
	connect_bd_net [get_bd_pins verification_ip_0/s00_axis_aresetn] [get_bd_pins sys_reset_0/peripheral_aresetn]
	connect_bd_net [get_bd_pins verification_ip_0/m00_axis_aclk] [get_bd_pins sys_clk_0/clk_out1]
	connect_bd_net [get_bd_pins verification_ip_0/m00_axis_aresetn] [get_bd_pins sys_reset_0/peripheral_aresetn]


	# Auto assign address
	assign_bd_address

	# Configure address param & range of verification_ip_0 master interfaces..
	set_property range 16K [get_bd_addr_segs {jtag_axi_0/Data/SEG_axi_bram_ctrl_0_Mem0}]
	set_property range 16K [get_bd_addr_segs {verification_ip_0/M00_AXI/SEG_axi_bram_ctrl_0_Mem0}]
	set_property -dict [list  CONFIG.C_M00_AXI_TARGET_SLAVE_BASE_ADDR {0xC0000000} ] [get_bd_cells verification_ip_0]

	# Copy all address to verification_ip_v1_0_include.tcl file
	set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
	upvar 1 $offsetfile offset_file
	set offset_file "${bd_path}/verification_ip_v1_0_include.tcl"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores user.org:user:verification_ip:1.0]]]]
set hw_test_file ${ip_path}/example_designs/debug_hw_design/verification_ip_v1_0_hw_test.tcl
...
for { set i 1 } { 1 } { incr i } {
	set design_name "verification_ip_v1_0_hw_${i}"
